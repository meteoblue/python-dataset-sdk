...
class FileCache(Cache):
    ...
    async def set(self, query_params, value):
        ...
        if self._get_valid_caches(query_dir_path):
            return
    ...
    async def get(self, query_params):
        ...
        if not os.path.exists(query_dir_path):
            return
        valid_caches = self._get_valid_caches(query_dir_path)
        if not valid_caches:
            return
        try:
            most_recent_file = valid_caches[0]
            ...
            print(os.path.join(self.cache_path, valid_caches[0]), "not found")
            return
    ...
    def _get_valid_caches(self, dir_path: str):
        return sorted(
            [
                file
